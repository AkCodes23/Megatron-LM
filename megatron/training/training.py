...
def _mamba_layer_flops(batch_size, seq_len, hidden_size, state_dim=16,
    ...
    # Input projection
    # Output dim = z(d_inner) + x(d_inner) + B(ngroups*d_state) + C(ngroups*d_state) + dt(nheads)
    in_proj_out_dim = 2 * d_inner + 2 * num_groups * state_dim + num_heads
    in_proj_flops = 2 * batch_size * seq_len * d_model * in_proj_out_dim

    # Convolution (depthwise)
    # Channels = x + B + C = d_inner + 2 * num_groups * state_dim
    conv_channels = d_inner + 2 * num_groups * state_dim
    d_conv = 4
    conv_flops = 2 * batch_size * seq_len * conv_channels * d_conv

    # SSM Scan
    ssm_flops = 7 * batch_size * seq_len * d_inner * state_dim

    # Output projection
    out_proj_flops = 2 * batch_size * seq_len * d_inner * d_model

    return in_proj_flops + conv_flops + ssm_flops + out_proj_flops
